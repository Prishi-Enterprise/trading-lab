...
The local LaunchAgent `in.prishi.tradinglab.nse-paper-week-2026` checks at **18:30 and 20:30 IST** on 25, 28, 29, 30 September and 1 October. The second check retries if the NSE archive was late; an existing snapshot is never overwritten. The runner refuses to record before 16:00 IST or outside those dates. It logs each attempt in ignored `state/nse-only/automation/checks.jsonl` and retires the installed LaunchAgent after the final 1 October check, or on the next login if that check was missed. The Mac must be awake and logged in for scheduled checks; a missed day cannot be filled retrospectively. A blocked check needs review of the log and source data. This is a local operating-system schedule, not a Codex notification or a live-trading service.
